		...
		try {
			...
			if (subcommand === 'włącz') {
				...
				if (addedChannels.length > 0) {
					const channelList = addedChannels.map(ch => `<#${ch.id}>`).join(', ');
					await interaction.reply({
						content: `🔒 **Blokowanie wiadomości włączone** w kanałach: ${channelList}\n\n` +
				...
				if (removedChannels.length > 0) {
					const channelList = removedChannels.map(ch => `<#${ch.id}>`).join(', ');
					await interaction.reply({
				...
				if (blockedChannels.length === 0) {
					await interaction.reply({
						content: '📋 **Brak zablokowanych kanałów**\n\nŻaden kanał nie ma włączonego blokowania wiadomości.',
						flags: 64 // ephemeral
					});
				} else {
					const channelList = blockedChannels.map(channelId => `<#${channelId}>`).join('\n• ');
					await interaction.reply({
						content: `📋 **Zablokowane kanały** (${blockedChannels.length}):\n\n• ${channelList}\n\n` +
