...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription(
      "Potwierdź usprawiedliwienie nieobecności ucznia (tylko w wątku nieobecności)"
    )
  ...
  async execute(interaction) {
    ...
    try {
      ...
      if (!interaction.channel.isThread()) {
        await interaction.reply({
          content:
            "❌ Ta komenda może być używana tylko w wątkach nieobecności.",
          flags: MessageFlags.Ephemeral,
        });
        return;
      }
      ...
      const isTeacher = teachers.some(
        (teacher) => teacher.discord_id === interaction.user.id
      );
      ...
      const threadMember = await interaction.channel.members
      ...
      try {
        ...
        await interaction.editReply({
          content:
            "❌ Wystąpił błąd podczas zapisywania usprawiedliwienia. Sprawdź logi.",
        });
      }
    } catch (error) {
