...
async function deployWithPermissions() {
  try {
    console.log(
      `[PERMISSIONS] Publikuję ${commands.length} komend(y) na serwer...`
    );
      ...
      if (permissions.length > 0) {
        ...
        permissions.forEach((perm) => {
          const roleName = Object.keys(roleMap).find(
            (name) => roleMap[name] === perm.id
          );
          console.log(
            `  - ${roleName}: ${perm.permission ? "DOZWOLONE" : "ZABLOKOWANE"}`
          );
        });
        ...
        try {
          ...
          console.log(
            `[PERMISSIONS] ✅ Ustawiono uprawnienia dla /${commandName}`
          );
        } catch (error) {
          console.error(
            `[PERMISSIONS] ❌ Błąd dla /${commandName}:`,
            error.message
          );
        }
      } else {
