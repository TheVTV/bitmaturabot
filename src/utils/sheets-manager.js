...
/**
 * Znajduje ucznia w arkuszu na podstawie imienia i nazwiska
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      if (fullName.includes(imie) && fullName.includes(nazwisko)) {
        console.log(`Znaleziono ucznia "${fullName}" w arkuszu grupy ${groupNumber}, wiersz ${i + 2} (imię: "${imie}", nazwisko: "${nazwisko}")`);
        return { row: i + 2, found: true };
      }
    }

    console.log(`Nie znaleziono ucznia "${fullName}" w arkuszu grupy ${groupNumber}`);
...
async function findDateColumn(groupNumber, targetDate) {
  try {
    ...
    for (let i = 0; i < dates.length; i++) {
      const dateInSheet = dates[i] && typeof dates[i] === 'string' ? dates[i].trim() : '';
      
      if (dateInSheet) {
        // Normalizuj datę z arkusza do formatu DD-MM-YYYY
        const [day, month, year] = dateInSheet.split('-');
        if (day && month && year) {
          const normalizedSheetDate = `${day.padStart(2, '0')}-${month.padStart(2, '0')}-${year}`;
    ...
    console.log(`Nie znaleziono daty "${targetDate}" w arkuszu grupy ${groupNumber}`);
    return { column: null, found: false };
  } catch (error) {
    console.error(`Błąd podczas szukania kolumny daty w arkuszu grupy ${groupNumber}:`, error);
    return { column: null, found: false };
  }
}
 ...
 * @param {string|number} groupNumber - Numer grupy
 * @param {string} fullName - Pełne imię i nazwisko ucznia
 * @param {string} absenceDate - Data nieobecności w formacie DD-MM-YYYY
 * @returns {Promise<{success: boolean, message: string}>}
 */
async function writeAbsenceToSheet(groupNumber, fullName, absenceDate) {
  try {
    if (!SPREADSHEET_ID) {
      return { 
        success: false, 
    ...
    // Znajdź ucznia w arkuszu
    const studentResult = await findStudentInSheet(groupNumber, fullName);
    if (!studentResult.found) {
      return { 
        success: false, 
        message: `Nie znaleziono ucznia "${fullName}" w arkuszu grupy ${groupNumber}` 
      };
    }
    ...
    console.log(`Zapisano usprawiedliwienie dla ${fullName} na datę ${absenceDate} w komórce ${cellAddress} (Grupa ${groupNumber})`);
    ...
    console.error("Błąd podczas zapisywania usprawiedliwienia w arkuszu:", error);
    return { 
      success: false, 
